...
# Build include paths and file map
include_paths = []
file_map = {}
...
def clean_eeg(subject_id, task_id, save_plot_dir=None):
    ...
    if len(reject_log.bad_epochs) > 0:
        ...
        fig = epochs[reject_log.bad_epochs].plot(scalings=dict(eeg=100e-6), show=False)
        if save_plot_dir:
            fig.savefig(os.path.join(save_plot_dir, f"sub-{subject_id}_task-{task_id}_bad_epochs.png"))
        plt.show()
    ...
    fig_evoked.suptitle(f"Evoked Response - Subject {subject_id} Task {task_id}")
    if save_plot_dir:
    ...
    if len(reject_log.bad_epochs) > 0:
        ...
        plt.title(f"Bad vs Clean Evoked - Subject {subject_id} Task {task_id}")
        if save_plot_dir:
