...
def clean_eeg(subject_id, task_id, config=None, save_dir=None):
    ...
    # 5) (Optional) save/preview ICA component maps & properties to help manual choice
    dpi = int(config.get("dpi", 200))
    show_plots = bool(config.get("show_ica_plots", False))
    try:
        figs = ica.plot_components(inst=epochs_for_ica, show=show_plots)
        figs = figs if isinstance(figs, (list, tuple)) else [figs]
        if not show_plots:
            for j, f in enumerate(figs):
                topo_path = os.path.join(save_plot_dir, f"sub-{subject_id}_task-{task_id}_ica_components_{j}.png")
                f.savefig(topo_path, dpi=dpi, bbox_inches="tight"); plt.close(f)
            print(f"Saved ICA component maps to {save_plot_dir}")
    except Exception as e:
        print(f"Could not render/save ICA component maps: {e}")

    n_props = int(config.get("ica_props_to_save", 0))  # e.g., 8 to save IC 0..7
    for k in range(min(n_props, getattr(ica, "n_components_", 0))):
        try:
            figs = ica.plot_properties(epochs_for_ica, picks=k, psd_args=dict(fmax=100.), show=False)
            figs = figs if isinstance(figs, (list, tuple)) else [figs]
            for f in figs:
                pth = os.path.join(save_plot_dir, f"sub-{subject_id}_task-{task_id}_ica_prop_{k}.png")
                f.savefig(pth, dpi=dpi, bbox_inches="tight"); plt.close(f)
        except Exception as e:
            print(f"Could not save properties for IC {k}: {e}")

    # 6) Manual exclusion ONLY (no ICLabel)
    manual = config.get("manual_ic_exclude", [])
    try:
        manual = [int(i) for i in manual]
    except Exception as e:
        print(f"manual_ic_exclude parsing failed: {e}"); manual = []

    # clamp to valid range
    max_ic = getattr(ica, "n_components_", 0)
    manual_valid = sorted({i for i in manual if 0 <= i < max_ic})
    invalid = sorted(set(manual) - set(manual_valid))
    if invalid:
        print(f"Warning: Skipping invalid IC indices (out of range 0..{max_ic-1}): {invalid}")

    # >>> Print exactly which ICs YOU chose to remove <<<
    print(f"Manually chosen ICs to exclude: {manual_valid}")

    ica.exclude = manual_valid  # set manual exclusions only

    # 7) Apply ICA (actual removal) + confirmation print
    raw_clean = ica.apply(raw_clean.copy())
    epochs_clean = ica.apply(epochs_clean.copy())
    print(f"Removed ICs (applied): {ica.exclude}")

    # 8) Evoked & save diagnostic
    ...
    try:
        fig_evoked.savefig(fig_ev_path, dpi=dpi, bbox_inches="tight"); print(f"Saved evoked plot to {fig_ev_path}")
    finally:
    ...
    if n_bad > 0:
        try:
            ...
            plt.gcf().savefig(bad_clean_path, dpi=dpi, bbox_inches="tight"); plt.close()
